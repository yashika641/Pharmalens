"""
Standalone test: OCR.Space API (Layer 2) — OCR metrics only
Run from backend/: python test_ocrspace.py [path/to/test_images]
Results saved to: backend/results/ocrspace_results_<timestamp>.xlsx
"""

import sys
import re
import io
import os
import csv
import time
from pathlib import Path
from datetime import datetime
from dotenv import load_dotenv

load_dotenv(Path(__file__).parent / ".env")
sys.path.insert(0, str(Path(__file__).parent.parent))

from backend.models.ocr.easy_ocr import run_ocr_space
from PIL import Image as PILImage

TEST_DIR    = Path(sys.argv[1]) if len(sys.argv) > 1 else Path(__file__).parent / "test_images"
RESULTS_DIR = Path(__file__).parent / "results"
SUPPORTED   = {".jpg", ".jpeg", ".png", ".bmp", ".webp", ".tiff"}
MAX_SIZE_KB = 900

# ── Helpers ────────────────────────────────────────────────────────────────────
def trunc(text: str, n: int = 300) -> str:
    t = text.replace("\n", " ").strip()
    return t[:n] + "…" if len(t) > n else t

def sep(char="─", w=70): print(char * w)
def hdr(t): sep("═"); print(f"  {t}"); sep("═")

def compress(image_bytes: bytes, max_kb: int = MAX_SIZE_KB) -> bytes:
    img = PILImage.open(io.BytesIO(image_bytes)).convert("RGB")
    quality = 92
    while quality >= 40:
        buf = io.BytesIO()
        img.save(buf, format="JPEG", quality=quality)
        data = buf.getvalue()
        if len(data) <= max_kb * 1024:
            return data
        quality -= 8
    return data

# ── CSV export ─────────────────────────────────────────────────────────────────
CSV_FIELDS = ["file", "status", "confidence", "elapsed",
              "original_kb", "compressed_kb",
              "chars", "words", "lines", "chars_per_sec", "preview"]

def export_csv(results: list, out_path: Path):
    out_path.parent.mkdir(exist_ok=True)
    with open(out_path, "w", newline="", encoding="utf-8") as f:
        writer = csv.DictWriter(f, fieldnames=CSV_FIELDS)
        writer.writeheader()
        for r in results:
            writer.writerow({
                "file":          r["file"],
                "status":        "PASS" if r["success"] else "FAIL",
                "confidence":    round(r["confidence"], 4),
                "elapsed":       round(r["elapsed"], 3),
                "original_kb":   round(r["original_kb"], 1),
                "compressed_kb": round(r["compressed_kb"], 1),
                "chars":         r["chars"],
                "words":         r["words"],
                "lines":         r["lines"],
                "chars_per_sec": round(r["chars_per_sec"], 1),
                "preview":       r["preview"],
            })
    print(f"\n💾 CSV saved → {out_path}")

# ── Main ───────────────────────────────────────────────────────────────────────
def main():
    api_key = os.getenv("OCR_SPACE_API_KEY", "")
    if not api_key:
        print("❌ OCR_SPACE_API_KEY not set in .env"); sys.exit(1)

    hdr("OCR.Space Layer 2 — Prescription OCR Test")
    print(f"  Folder  : {TEST_DIR}")
    print(f"  API key : {api_key[:6]}{'*'*(len(api_key)-6)}\n")

    if not TEST_DIR.exists():
        print(f"❌ Folder not found: {TEST_DIR}"); sys.exit(1)

    images = sorted(f for f in TEST_DIR.iterdir() if f.suffix.lower() in SUPPORTED)
    if not images:
        print(f"❌ No images in {TEST_DIR}"); sys.exit(1)

    print(f"  {len(images)} image(s) found\n")
    results = []

    for img_path in images:
        sep()
        print(f"  📄 {img_path.name}")
        sep()

        raw_bytes    = img_path.read_bytes()
        original_kb  = len(raw_bytes) / 1024

        if original_kb > MAX_SIZE_KB:
            image_bytes   = compress(raw_bytes)
            compressed_kb = len(image_bytes) / 1024
            print(f"  🗜  Compressed : {original_kb:.0f} KB → {compressed_kb:.0f} KB")
        else:
            image_bytes   = raw_bytes
            compressed_kb = original_kb
            print(f"  📦 Size       : {original_kb:.0f} KB")

        start = time.perf_counter()
        try:
            text, confidence = run_ocr_space(image_bytes)
            elapsed = time.perf_counter() - start
        except Exception as e:
            elapsed = time.perf_counter() - start
            print(f"  ❌ {e}")
            results.append({
                "file": img_path.name, "success": False,
                "confidence": 0.0, "elapsed": round(elapsed, 3),
                "original_kb": round(original_kb, 1), "compressed_kb": round(compressed_kb, 1),
                "chars": 0, "words": 0, "lines": 0,
                "chars_per_sec": 0.0, "preview": "",
            })
            continue

        chars         = len(text)
        words         = len(text.split())
        lines         = len([l for l in text.splitlines() if l.strip()])
        chars_per_sec = chars / elapsed if elapsed > 0 else 0

        print(f"  ✅ Confidence  : {confidence:.3f}")
        print(f"  ⏱  Time        : {elapsed:.2f}s")
        print(f"  📝 Chars       : {chars}   Words: {words}   Lines: {lines}")
        print(f"  🚀 Chars/sec   : {chars_per_sec:.0f}")
        print(f"\n  Preview : {trunc(text)}\n")

        results.append({
            "file": img_path.name, "success": True,
            "confidence": confidence, "elapsed": round(elapsed, 3),
            "original_kb": round(original_kb, 1), "compressed_kb": round(compressed_kb, 1),
            "chars": chars, "words": words, "lines": lines,
            "chars_per_sec": round(chars_per_sec, 1), "preview": trunc(text, 500),
        })

    # Console summary
    print(); hdr("SUMMARY — OCR.Space")
    print(f"  {'File':<32} {'Status':>6} {'Conf':>7} {'Time':>7} {'Chars':>7} {'Words':>6} {'Lines':>6}")
    sep()

    sr = []
    for r in results:
        st = "PASS" if r["success"] else "FAIL"
        print(f"  {r['file']:<32} {st:>6} {r['confidence']:>7.3f} "
              f"{r['elapsed']:>6.2f}s {r['chars']:>7} {r['words']:>6} {r['lines']:>6}")
        if r["success"]: sr.append(r)

    n = len(sr)
    if n:
        sep()
        print(f"  {'AVERAGE':<32} {'':>6} "
              f"{sum(r['confidence'] for r in sr)/n:>7.3f} "
              f"{sum(r['elapsed'] for r in sr)/n:>6.2f}s "
              f"{sum(r['chars'] for r in sr)//n:>7} "
              f"{sum(r['words'] for r in sr)//n:>6} "
              f"{sum(r['lines'] for r in sr)//n:>6}")

    print(f"\n  Total: {len(images)}  Pass: {n}  Fail: {len(images)-n}")
    sep("═")

    ts = datetime.now().strftime("%Y%m%d_%H%M%S")
    export_csv(results, RESULTS_DIR / f"ocrspace_prescription_{ts}.csv")


if __name__ == "__main__":
    main()
